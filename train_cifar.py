...
parser = argparse.ArgumentParser(description='Barlow Twins Training', formatter_class=argparse.RawTextHelpFormatter)
...
def main_worker(gpu, args):
    ...
    if args.loss == 'sacm':
        model = BarlowTwinsSACM(args).cuda(gpu)
    elif args.loss == 'bacm':
        model = BarlowTwinsBACM(args).cuda(gpu)
    else:
    ...
    transform = Transform(args.dataset)

    if args.dataset == 'cifar10':
        dataset = torchvision.datasets.CIFAR10(args.data / 'train', transform=transform, download=True, train=True)
    else:
        dataset = torchvision.datasets.CIFAR100(args.data / 'train', transform=transform, download=True, train=True)
...
class BarlowTwinsSACM(BarlowTwins):
    def forward(self, y1, y2):
        z1 = self.projector(self.backbone(y1))
        z2 = self.projector(self.backbone(y2))
        ...
        a = self.bn(z1).T @ self.bn(z1)

        # sum the cross-correlation matrix between all gpus
        c.div_(self.args.batch_size)
        torch.distributed.all_reduce(c)
        
        # sum auto-correlation matrix between all gpus
        a.div_(self.args.batch_size)
        torch.distributed.all_reduce(a)

        on_diag = torch.diagonal(c).add_(-1).pow_(2).sum()
        
        #off_diag = off_diagonal(c).pow_(2).sum()
        off_diag = off_diagonal(a).pow_(2).sum()
        
        loss = on_diag + self.args.lambd * off_diag
        return loss

class BarlowTwinsBACM(BarlowTwins):
    def forward(self, y1, y2):
        z1 = self.projector(self.backbone(y1))
        z2 = self.projector(self.backbone(y2))
        ...
        a1 = self.bn(z1).T @ self.bn(z1)
        a2 = self.bn(z2).T @ self.bn(z2)
        ...
        # sum auto-correlation matrix between all gpus
        a1.div_(self.args.batch_size)
        a2.div_(self.args.batch_size)
        torch.distributed.all_reduce(a1)
        torch.distributed.all_reduce(a2)

        on_diag = torch.diagonal(c).add_(-1).pow_(2).sum()
        
        #off_diag = off_diagonal(c).pow_(2).sum()
        off_diag1 = off_diagonal(a1).pow_(2).sum()
        off_diag2 = off_diagonal(a2).pow_(2).sum()
        
        loss = on_diag + (self.args.lambd/2) * (off_diag1 + off_diag2)
        return loss
...
class Transform:
    ...
    def __init__(self, dataset):
        ...
        self.transform = transforms.Compose([
            ...
            Solarization(p=0.0),
            transforms.ToTensor(),
            transforms.Normalize(mean=self.mean[dataset], std=self.std[dataset])
        ])
